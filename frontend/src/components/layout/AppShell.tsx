...
export function AppShell({ children }: AppShellProps) {
  ...
  return (
    <EvidenceProvider>
      <CockpitProvider>
        <div className="flex h-screen w-screen overflow-hidden bg-[#F6F7F9] text-[#111318] font-sans">
          {/* Workflow Stages Sidebar */}
          <AutoGTMWorkflowSidebar />

          {/* Right Main Column (Top Stepper + Unified Cockpit Area) */}
          <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-[#F6F7F9]">
            <CockpitHeader />
            <main className="flex-1 min-h-0 overflow-hidden flex flex-col bg-[#F6F7F9]">
              {children}
            </main>
          </div>
